...
| Done | 파일 업로드 검증 로직 자동화 테스트 보강 | 악성 파일 업로드와 원본 파일명 기반 key 오염을 막아야 한다. | SR-FILE-001, TM-FILE-001 | Partially automated: `ImageControllerSecurityTest`, `S3ImageServiceTest`에서 ROLE_MEMBER 차단, 확장자/MIME/크기/빈 파일, S3 key 일부 검증 |
| Needs Verification | S3 bucket/IAM/운영 업로드 설정 확인 | bucket 공개 범위, IAM 최소 권한, staging/prod 업로드 설정은 코드 테스트만으로 확인할 수 없다. | SR-FILE-001, TM-FILE-001, TM-S3-001 | 실제 secret 값 없이 S3 bucket public access, IAM policy, staging/prod 업로드 동작을 수동 점검 |
